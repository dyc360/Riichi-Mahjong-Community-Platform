...
class MajSoulNews(models.Model):
    """
    雀魂新闻数据模型
    存储从雀魂官网爬取的新闻数据
    """
    title = models.CharField(max_length=300, verbose_name="标题", db_index=True)
    description = models.TextField(blank=True, verbose_name="描述")
    link = models.URLField(verbose_name="原文链接", unique=True, db_index=True)
    image_url = models.URLField(blank=True, verbose_name="图片链接")
    published_at = models.DateTimeField(verbose_name="发布时间", db_index=True)
    category = models.CharField(max_length=50, blank=True, verbose_name="分类", help_text="如: maintenance, events, news等")
    source = models.CharField(max_length=100, default="雀魂官网", verbose_name="来源")
    is_active = models.BooleanField(default=True, verbose_name="是否有效", db_index=True)
