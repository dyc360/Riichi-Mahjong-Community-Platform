...
from .models import ForumSection, ForumPost, ForumReply, PostLike, ReplyLike
...
class ForumPostCreateView(generics.CreateAPIView):
    ...
    # 在视图内部手动检查 JWT，这里不再依赖 DRF 的权限系统
    permission_classes = [permissions.AllowAny]
...
def toggle_reply_like(request, reply_id: int):
    ...
    try:
        ...
        # 确保likes字段和ReplyLike记录数量同步（防止数据不一致）
