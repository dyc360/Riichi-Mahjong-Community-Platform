...
def generate_efficiency_hand(num: int):
    ...
    while num > 0:
        tile_choice = random.randint(9, 17)  # 只选择万子、筒子、索子
...
def generate_ukeire(tile_num: list[int]) -> int:
    ukeire_tile = -1
    while True:
        ukeire_tile = random.randint(9, 17)
...
# 主处理程序
def process_efficiency_chinitsu(tile_34_array: list[int], tile_num: list[int], tile_13_in_hand: list[int],
