    ...
    def calculate_fu(
        ...
        if win_group in closed_chi_sets:
            ...
            if contains_terminals(win_group):
                ...
                if tile_index == 2 and win_group.index(win_tile_34) == 2:
                    fu_details.append({"fu": 2, "reason": FuCalculator.PENCHAN, "tile" : win_tile_34})
                # 8-9-... wait
            ...
            if win_group.index(win_tile_34) == 1:
                fu_details.append({"fu": 2, "reason": FuCalculator.KANCHAN, "tile" : win_tile_34})

        # valued pair
        count_of_valued_pairs = valued_tiles.count(pair[0])
        if count_of_valued_pairs == 1:
            fu_details.append({"fu": 2, "reason": FuCalculator.VALUED_PAIR, "tile" : pair[0]})

        # east-east pair when you are on east gave double fu
        if count_of_valued_pairs == 2:
            fu_details.append({"fu": 4, "reason": FuCalculator.DOUBLE_VALUED_PAIR, "tile" : pair[0]})
        # pair wait
        if is_pair(win_group):
            fu_details.append({"fu": 2, "reason": FuCalculator.PAIR_WAIT, "tile" : win_group[0]})
        ...
        for set_item in pon_sets:
            ...
            if is_honor:
                if is_kan_set:
                    if set_was_open:
                        fu_details.append({"fu": 16, "reason": FuCalculator.OPEN_TERMINAL_KAN, "tile" : set_item[0]})
                    else:
                        fu_details.append({"fu": 32, "reason": FuCalculator.CLOSED_TERMINAL_KAN, "tile" : set_item[0]})
                else:
                    if set_was_open:
                        fu_details.append({"fu": 4, "reason": FuCalculator.OPEN_TERMINAL_PON, "tile" : set_item[0]})
                    else:
                        fu_details.append({"fu": 8, "reason": FuCalculator.CLOSED_TERMINAL_PON, "tile" : set_item[0]})
            else:
                if is_kan_set:
                    if set_was_open:
                        fu_details.append({"fu": 8, "reason": FuCalculator.OPEN_KAN, "tile" : set_item[0]})
                    else:
                        fu_details.append({"fu": 16, "reason": FuCalculator.CLOSED_KAN, "tile" : set_item[0]})
                else:
                    if set_was_open:
                        fu_details.append({"fu": 2, "reason": FuCalculator.OPEN_PON, "tile" : set_item[0]})
                    else:
                        fu_details.append({"fu": 4, "reason": FuCalculator.CLOSED_PON, "tile" : set_item[0]})
        ...
        # for x in fu_details:
        #     fu_sum += x.get("fu", 0)
        # fu_details.append({"fu": 0, "fu_sum": fu_sum})
