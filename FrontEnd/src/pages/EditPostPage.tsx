...
export default function EditPostPage() {
	const { user } = useAuth();
	const { sections, fetchSections, updatePost, fetchPostDetailByTitle, fetchPosts } = useForum();
	...
	useEffect(() => {
		const loadPost = async () => {
			...
			try {
				...
				const post = await fetchPostDetailByTitle(decodedTitle);
				...
				setFormData({
					title: post.title,
					sectionId: sections.find(s => s.name === post.section_name)?.id || 0,
	...
	}, [title, sections, fetchPostDetailByTitle, user]);

	// 设置默认板块
	useEffect(() => {
		if (sections.length > 0 && formData.sectionId === 0 && formData.title) {
			// 如果已经有帖子数据，不自动设置默认板块
			return;
		}
	...
	}, [sections, formData.sectionId, formData.title]);

	// 检查用户是否登录
	if (!user) {
		navigate('/login?redirect=/forum/edit-post/' + encodeURIComponent(title || ''), { replace: true });
		return null;
	}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const errorMessage = error?.message || "更新帖子失败，请稍后重试";
			alert(errorMessage);
		} finally {
	...
	return (
		<>
			...
			<main className="container mx-auto px-4 py-8">
				<h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-8">编辑帖子</h1>
						...
						<ModuleContainer title="">
							<form onSubmit={handleSubmit} className="space-y-6">
								...
								<div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
									<Link
										to={title ? `/forum/post/${encodeURIComponent(title)}` : "/forum"}
