...
import { HOT_POSTS } from './ForumPage';
import type { ForumPost } from './ForumSectionPage';

// 帖子项组件
const HotPostItem = ({ post, isHot }: { post: ForumPost & { lastReply?: { author: string; time: string } }, isHot: boolean }) => {
    return (
...
export default function ForumHotPage() {
    ...
    useEffect(() => {
        const fetchHotPosts = async () => {
            try {
                ...
                const sortedPosts = [...HOT_POSTS].sort((a, b) => b.views - a.views);
                ...
                <main className="container mx-auto px-4 py-8">
                    <div className="flex justify-center items-center h-60">
                        <div className="text-center">
                            ...
                            <p className="text-slate-600 dark:text-slate-400">加载热门帖子中...</p>
                        </div>
                    </div>
                </main>
            </>
        ...
        <>
            ...
            <main className="container mx-auto px-4 py-8">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white">热门帖子</h1>
                    <Link
                ...
                <ModuleContainer title="热门帖子列表" description="按浏览量排序的热门讨论内容">
                    {currentPosts.length > 0 ? (
                        <>
                            <div className="space-y-4">
                                {currentPosts.map(post => (
                                    <HotPostItem key={post.id} post={post} isHot={true} />
                                ))}
                            </div>
                        ...
                        <div className="py-12 text-center">
                            <p className="text-slate-500 dark:text-slate-400">暂无热门帖子数据</p>
                        </div>
                    )}
                </ModuleContainer>
            </main>
